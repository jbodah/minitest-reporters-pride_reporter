...
task :full_deploy => [:spec, :change_version] do
...
task :change_version do
  ...
  version_file = 'lib/minitest/reporters/pride_reporter.rb'
  before_text = File.read(version_file)
  text = before_text.gsub(/(?<=VERSION = ').+(?=')/, ENV['TO'])
  raise "Aborting: Version didn't change" if text == before_text
  File.open(version_file, 'w') {|f| f.puts text}

  puts 'Committing version.rb'
  exit(1) unless system "git add lib/minitest/reporters/pride_reporter.rb"
